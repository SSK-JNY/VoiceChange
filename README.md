...
- **Retrieval-based Voice Conversion**
  - 事前学習済みモデルを使用したAI音声変換
  - リアルタイム処理対応
  - **現バージョン: 簡易実装 (librosaベース)**
  - 本物のRVC品質が必要な場合はVCClient等の専用ツールを使用してください

- **特徴**
  - 学習済みモデルによる音色変換（準備中）
  - ピッチシフト機能付き (-24～+24 セミトーン)
  - GPUアクセラレーション対応（準備中）
  - 低遅延リアルタイム処理

- **制限事項**
  - 完全なRVCニューラルネットワークは未実装
  - 現在の実装は基本的なピッチシフトのみ
  - VCClient等の専用ツールほどの品質は期待できません
...
```bash
pip install sounddevice pedalboard numpy scipy
```
...
| パッケージ    | 用途                                     | バージョン |
| ------------- | ---------------------------------------- | ---------- |
| `sounddevice` | リアルタイム音声入出力                   | ≥0.4.5     |
| `pedalboard`  | オーディオエフェクト (PitchShift)        | ≥0.7.0     |
| `numpy`       | 数値演算、配列処理                       | ≥1.20      |
| `scipy`       | 信号処理 (FFT, フォルマント、ノイズ除去) | ≥1.7       |

### RVC機能を使用する場合

RVC (Retrieval-based Voice Conversion) を使用するには、追加の依存関係が必要です：

```bash
pip install torch torchaudio torchvision fairseq librosa faiss-cpu gradio scikit-learn pyworld praat-parselmouth
```
...
| パッケージ   | 用途                           | バージョン |
| ------------ | ------------------------------ | ---------- |
| `torch`      | PyTorch 深層学習フレームワーク | ≥1.13.0    |
| `torchaudio` | PyTorch音声処理                | ≥0.13.0    |
| `fairseq`    | 音声変換モデル                 | ≥0.12.2    |
| `librosa`    | 音声分析・処理                 | ≥0.9.0     |
| `faiss-cpu`  | ベクトル検索                   | ≥1.7.0     |
| `gradio`     | Web UI (オプション)            | ≥3.0.0     |
...
現在の簡易実装ではVCClient等の専用ツールほどの品質は得られません。本物のRVC変換が必要な場合は：

#### オプション1: VCClientの使用（推奨）

```bash
# VCClientを別途インストールして使用
# https://github.com/RVC-Project/Retrieval-based-Voice-Conversion-WebUI
```

#### オプション2: 完全RVC実装の開発

完全なRVCを実装するには以下の作業が必要です：
...
**注意**: 完全実装は非常に複雑で、専門知識を要します。

### 品質比較

| 機能           | 現在の簡易実装       | VCClient等の本物RVC    |
| -------------- | -------------------- | ---------------------- |
| 音質           | 基本的なピッチシフト | AIによる自然な音声変換 |
| リアルタイム性 | ○                    | ○                      |
| セットアップ   | 簡単                 | 複雑                   |
| モデル品質     | △                    | ◎                      |
| カスタムモデル | △                    | ◎                      |
...
config.py の BLOCKSIZE を増加

```python
BLOCKSIZE = 4096  # デフォルトから 8192 に増加
```
