...
- numpy バージョンは現状 `requirements.txt` に従います（固定指定なし）
...
- blocksize
  - オーディオチャンクサイズ（サンプル数）
  - 大きいほど安定する代わりに遅延が増加
  - コード上の既定値: 1024（`src/app/gui_local_settings.py`）
  - 設定ファイル（`gui_local_settings.json`）が存在する場合はその値が優先されます
  - サンプルレート 44.1kHz 時の体感遅延目安：

| blocksize | 遅延(ms) | 用途                         |
| --------- | -------- | ---------------------------- |
| 512       | 11.6     | 超低遅延（リアルタイム会話） |
| 1024      | 23.2     | 低遅延（会話重視）           |
| 2048      | 46.4     | 低遅延（バランス）           |
| 4096      | 92.9     | 標準（バランス）             |
| 8192      | 185.8    | 標準（安定性重視）           |
| 12288     | 278.6    | 高安定性（推奨）             |
| 16384     | 371.5    | 高安定性                     |
| 24576     | 557.3    | 最大安定性（完全変換利用時） |
| 32768     | 742.9    | 最大安定性                   |
| 44100     | 1000.0   | リアルタイム性不要時         |
...
- 初期値（現行）
  - `robot_distortion_drive_db = 0.0`
  - `robot_chorus_mix = 0.0`
  - 既定ではロボ化はオフ状態です
...
- モデルは「WSL 側で起動している同一リポジトリ」の `src/models/rvc/` に配置し、サーバ経由で利用してください

## オーディオモード

`AudioModel` は以下の3モードを持ちます。

- `normal`: ノイズ除去 / RVC / フォルマント / Pedalboard を適用
- `passthrough`: エフェクトを通さず入出力ゲインのみ適用
- `test-tone`: テスト用サイン波を出力
